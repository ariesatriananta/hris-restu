...
CREATE TABLE employees (
  ...
  CONSTRAINT chk_employees_education_level CHECK (education_level IS NULL OR education_level IN ('NO_SCHOOLING','NOT_COMPLETED_PRIMARY','PRIMARY','JUNIOR_SECONDARY','SENIOR_SECONDARY','DIPLOMA_I_II','DIPLOMA_III','DIPLOMA_IV_BACHELOR','MASTER','DOCTORATE')),
...
CREATE TABLE employee_salary_histories (
  ...
  UNIQUE KEY uq_employee_salary_employee (employee_id),
...
CREATE TABLE site_minimum_wages (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  uid CHAR(36) NOT NULL,
  site_id BIGINT UNSIGNED NOT NULL,
...
CREATE TABLE site_minimum_wage_revisions (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  uid CHAR(36) NOT NULL,
...
CREATE TABLE payroll_bpjs_policies (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  uid CHAR(36) NOT NULL,
  ...
  CONSTRAINT chk_payroll_bpjs_policy_flags CHECK (health_employer_enabled IN (0,1) AND health_employee_enabled IN (0,1) AND jht_employer_enabled IN (0,1) AND jht_employee_enabled IN (0,1) AND jkk_employer_enabled IN (0,1) AND jkm_employer_enabled IN (0,1) AND jp_employer_enabled IN (0,1) AND jp_employee_enabled IN (0,1)),
  CONSTRAINT chk_payroll_bpjs_policy_rates CHECK (health_employer_rate BETWEEN 0 AND 100 AND health_employee_rate BETWEEN 0 AND 100 AND jht_employer_rate BETWEEN 0 AND 100 AND jht_employee_rate BETWEEN 0 AND 100 AND jkk_employer_rate BETWEEN 0 AND 100 AND jkm_employer_rate BETWEEN 0 AND 100 AND jp_employer_rate BETWEEN 0 AND 100 AND jp_employee_rate BETWEEN 0 AND 100),
...
CREATE TABLE payroll_bpjs_policy_revisions (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  uid CHAR(36) NOT NULL,
...
-- Legacy: dipertahankan untuk referensi snapshot Payroll sebelum JKK global.
CREATE TABLE site_bpjs_settings (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  uid CHAR(36) NOT NULL,
  site_id BIGINT UNSIGNED NOT NULL,
...
CREATE TABLE site_bpjs_setting_revisions (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  uid CHAR(36) NOT NULL,
...
CREATE TABLE employee_bpjs_enrollment_revisions (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  uid CHAR(36) NOT NULL,
...
CREATE TABLE payroll_policy_versions (
  ...
  UNIQUE KEY uq_payroll_policy_scope (site_id,employee_type_code),
...
CREATE TABLE payroll_policy_revisions (
  ...
  CONSTRAINT chk_payroll_policy_revision_type CHECK (revision_type IN ('CREATE','CORRECTION','CANCELLATION','REACTIVATION')),
...
CREATE TABLE employee_daily_rate_histories (
  ...
  UNIQUE KEY uq_employee_daily_rate_employee (employee_id),
...
CREATE TABLE employee_daily_rate_revisions (
  ...
  CONSTRAINT chk_employee_daily_rate_revision_type CHECK (revision_type IN ('CREATE','CORRECTION','CANCELLATION','REACTIVATION')),
...
CREATE TABLE employee_salary_history_revisions (
  ...
  CONSTRAINT chk_employee_salary_revision_type CHECK (revision_type IN ('CREATE','CORRECTION','CANCELLATION','REACTIVATION')),
...
-- ============================================================================
-- C1. REKRUTMEN DAN FORM DATA PELAMAR
-- ============================================================================
...
CREATE TABLE recruitment_candidates (
  ...
  CONSTRAINT chk_recruitment_candidates_education_level CHECK (education_level IS NULL OR education_level IN ('NO_SCHOOLING','NOT_COMPLETED_PRIMARY','PRIMARY','JUNIOR_SECONDARY','SENIOR_SECONDARY','DIPLOMA_I_II','DIPLOMA_III','DIPLOMA_IV_BACHELOR','MASTER','DOCTORATE')),
...
CREATE TABLE recruitment_candidate_files (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  uid CHAR(36) NOT NULL,
...
CREATE TABLE attendance_daily_finalization_runs (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  uid CHAR(36) NOT NULL,
  site_id BIGINT UNSIGNED NOT NULL,
  business_date DATE NOT NULL,
...
CREATE TABLE attendance_classification_requests (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  uid CHAR(36) NOT NULL,
  employee_id BIGINT UNSIGNED NOT NULL,
  site_id BIGINT UNSIGNED NOT NULL,
...
CREATE TABLE attendance_classification_details (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  uid CHAR(36) NOT NULL,
...
CREATE TABLE payroll_periods (
  ...
  pay_frequency VARCHAR(20) NOT NULL DEFAULT 'WEEKLY',
  employee_type_code VARCHAR(30) NOT NULL,
  deduct_bpjs TINYINT(1) NOT NULL DEFAULT 0,
  bpjs_contribution_month DATE NULL,
  status VARCHAR(20) NOT NULL DEFAULT 'DRAFT',
  ...
  CONSTRAINT chk_payroll_period_bpjs CHECK ((deduct_bpjs=0 AND bpjs_contribution_month IS NULL) OR (deduct_bpjs=1 AND employee_type_code='BORONGAN' AND DAY(bpjs_contribution_month)=1)),
...
CREATE TABLE payroll_period_manual_component_revisions (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  uid CHAR(36) NOT NULL,
...
CREATE TABLE payroll_runs (
  ...
  total_basic_salary_amount DECIMAL(18,2) NOT NULL DEFAULT 0.00,
  ...
  CONSTRAINT chk_payroll_runs_totals CHECK (total_piece_rate_amount >= 0 AND total_basic_salary_amount >= 0 AND total_earnings >= 0 AND total_deductions >= 0),
...
CREATE TABLE payroll_employee_results (
  ...
  CONSTRAINT chk_payroll_employee_amounts CHECK (piece_rate_amount >= 0 AND basic_salary_amount >= 0 AND additional_earnings >= 0 AND gross_earnings >= 0 AND total_deductions >= 0),
...
CREATE TABLE payroll_bpjs_monthly_settlements (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  uid CHAR(36) NOT NULL,
  employee_id BIGINT UNSIGNED NOT NULL,
  site_id BIGINT UNSIGNED NOT NULL,
...
CREATE TABLE payroll_employee_bpjs_details (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  uid CHAR(36) NOT NULL,
  ...
  CONSTRAINT chk_payroll_employee_bpjs_amounts CHECK (minimum_wage_snapshot>0 AND rounding_unit_snapshot>0 AND health_employer_amount>=0 AND health_employee_amount>=0 AND jht_employer_amount>=0 AND jht_employee_amount>=0 AND jkk_employer_amount>=0 AND jkm_employer_amount>=0 AND jp_employer_amount>=0 AND jp_employee_amount>=0 AND total_employee_deduction>=0 AND total_employer_contribution>=0),
...
CREATE TABLE payroll_time_details (
  ...
  CONSTRAINT chk_payroll_time_payable CHECK ((is_payable=1 AND attendance_status_snapshot='PRESENT' AND amount_snapshot=daily_rate_snapshot) OR (is_payable=0 AND amount_snapshot=0)),
  CONSTRAINT chk_payroll_time_warning CHECK (warning_code IS NULL OR warning_code='OFFDAY_PRESENT'),
  CONSTRAINT fk_payroll_time_result FOREIGN KEY (payroll_employee_result_id) REFERENCES payroll_employee_results(id) ON UPDATE CASCADE ON DELETE RESTRICT,
  CONSTRAINT fk_payroll_time_attendance FOREIGN KEY (attendance_record_id) REFERENCES attendance_records(id) ON UPDATE CASCADE ON DELETE RESTRICT,
  CONSTRAINT fk_payroll_time_rate FOREIGN KEY (employee_daily_rate_history_id) REFERENCES employee_daily_rate_histories(id) ON UPDATE CASCADE ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE payroll_monthly_summaries (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  uid CHAR(36) NOT NULL,
  payroll_employee_result_id BIGINT UNSIGNED NOT NULL,
  employee_salary_history_id BIGINT UNSIGNED NOT NULL,
  ...
  CONSTRAINT chk_payroll_monthly_summary_days CHECK (period_calendar_days>0 AND eligible_calendar_days BETWEEN 1 AND period_calendar_days AND scheduled_work_days>=alpha_days+permission_days),
  CONSTRAINT chk_payroll_monthly_summary_amounts CHECK (full_basic_salary_snapshot>0 AND prorated_basic_salary>=0 AND alpha_deduction>=0 AND permission_deduction>=0),
  CONSTRAINT chk_payroll_monthly_summary_currency CHECK (currency_snapshot='IDR'),
  CONSTRAINT fk_payroll_monthly_summary_result FOREIGN KEY (payroll_employee_result_id) REFERENCES payroll_employee_results(id) ON UPDATE CASCADE ON DELETE RESTRICT,
  CONSTRAINT fk_payroll_monthly_summary_salary FOREIGN KEY (employee_salary_history_id) REFERENCES employee_salary_histories(id) ON UPDATE CASCADE ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE payroll_monthly_daily_details (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  uid CHAR(36) NOT NULL,
  payroll_employee_result_id BIGINT UNSIGNED NOT NULL,
  attendance_record_id BIGINT UNSIGNED NULL,
  ...
  CONSTRAINT chk_payroll_monthly_daily_deduction CHECK ((deduction_type='NONE') OR (deduction_type='ALPHA' AND is_scheduled=1 AND attendance_status_snapshot='ABSENT') OR (deduction_type='PERMISSION' AND is_scheduled=1 AND attendance_status_snapshot='PERMISSION')),
  ...
  CONSTRAINT fk_payroll_monthly_daily_result FOREIGN KEY (payroll_employee_result_id) REFERENCES payroll_employee_results(id) ON UPDATE CASCADE ON DELETE RESTRICT,
  CONSTRAINT fk_payroll_monthly_daily_attendance FOREIGN KEY (attendance_record_id) REFERENCES attendance_records(id) ON UPDATE CASCADE ON DELETE RESTRICT,
  CONSTRAINT fk_payroll_monthly_daily_salary FOREIGN KEY (employee_salary_history_id) REFERENCES employee_salary_histories(id) ON UPDATE CASCADE ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE payroll_training_production_details (
  ...
  UNIQUE KEY uq_payroll_training_result_transaction (payroll_employee_result_id,production_transaction_id),
  ...
  CONSTRAINT fk_payroll_training_production_result FOREIGN KEY (payroll_employee_result_id) REFERENCES payroll_employee_results(id) ON UPDATE CASCADE ON DELETE RESTRICT,
  CONSTRAINT fk_payroll_training_production_transaction FOREIGN KEY (production_transaction_id) REFERENCES production_transactions(id) ON UPDATE CASCADE ON DELETE RESTRICT,
  CONSTRAINT fk_payroll_training_production_job FOREIGN KEY (production_job_id) REFERENCES production_jobs(id) ON UPDATE CASCADE ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE payroll_workflow_actions (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  uid CHAR(36) NOT NULL,
  payroll_period_id BIGINT UNSIGNED NOT NULL,
  payroll_run_id BIGINT UNSIGNED NOT NULL,
...
INSERT INTO payroll_policy_versions(
  ...
  deduction_divisor,rounding_mode,rounding_scale,currency,status,notes
)
SELECT UUID(),s.id,m.employee_type_code,m.wage_basis,m.pay_frequency,m.cutoff_type,
       ...
       'HALF_UP',0,'IDR','ACTIVE','Policy awal M5A1.'
FROM sites s JOIN (
...
VALUES
  ...
  (UUID(), 'MONTHLY_ALPHA_DEDUCTION', 'Potongan Alpha Bulanan', 'DEDUCTION', 'FORMULA', 0, 'Potongan otomatis Alpha berdasarkan gaji pokok dan hari kerja terjadwal periode.'),
  (UUID(), 'MONTHLY_PERMISSION_DEDUCTION', 'Potongan Izin Bulanan', 'DEDUCTION', 'FORMULA', 0, 'Potongan otomatis Izin berdasarkan gaji pokok dan hari kerja terjadwal periode.'),
  (UUID(), 'BPJS_HEALTH_EMPLOYEE', 'BPJS Kesehatan Karyawan', 'DEDUCTION', 'FORMULA', 0, 'Potongan BPJS Kesehatan bagian karyawan dari snapshot sistem.'),
  (UUID(), 'BPJS_JHT_EMPLOYEE', 'JHT Karyawan', 'DEDUCTION', 'FORMULA', 0, 'Potongan JHT bagian karyawan dari snapshot sistem.'),
  (UUID(), 'BPJS_JP_EMPLOYEE', 'Jaminan Pensiun Karyawan', 'DEDUCTION', 'FORMULA', 0, 'Potongan JP bagian karyawan dari snapshot sistem.');
...
VALUES
  ...
  (UUID(), 'recruitment.view', 'recruitment', 'Lihat Rekrutmen'),
  (UUID(), 'recruitment.manage', 'recruitment', 'Kelola Rekrutmen'),
  ...
  (UUID(), 'attendance.finalize', 'attendance', 'Finalisasi Attendance'),
...
-- HR Officer mengelola Rekrutmen sesuai pembatasan site. Super Admin sudah
-- memperoleh permission ini melalui matriks seluruh permission di atas.
INSERT INTO role_permissions (uid, role_id, permission_id)
SELECT UUID(), r.id, p.id
FROM roles r
