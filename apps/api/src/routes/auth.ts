...
async function issue(req: Parameters<typeof clientInfo>[0], res: Parameters<typeof setAuthCookies>[0], user: { id: number; uid: string }) {
  ...
  const expires = new Date(Date.now() + env.REFRESH_TOKEN_TTL_DAYS * 86_400_000).toISOString().slice(0, 23).replace('T', ' ')
  await pool.execute(`INSERT INTO user_sessions(uid,user_id,refresh_token_hash,device_name,ip_address,user_agent,expires_at) VALUES(?,?,?,?,?,?,?)`, [sessionUid,user.id,hashToken(refresh),info.device ?? null,info.ip ?? null,info.userAgent ?? null,expires])
  setAuthCookies(res, await signAccessToken(user.uid, sessionUid), refresh)
}
export const authRouter = Router()
authRouter.post('/sign-in', async (req, res, next) => { try { const input = loginSchema.parse(req.body); const [rows] = await pool.query<RowDataPacket[]>('SELECT id,uid,email,username,password_hash,status,locked_until FROM users WHERE email=? OR username=? LIMIT 1',[input.email.toLowerCase(),input.email]); const user=rows[0]; if (!user || user.status !== 'ACTIVE' || (user.locked_until && new Date(user.locked_until)>new Date()) || !(await argon2.verify(user.password_hash,input.password))) throw new ApiError(401,'Email atau kata sandi tidak sesuai.'); await issue(req,res,{id:user.id,uid:user.uid}); await pool.execute('UPDATE users SET failed_login_attempts=0,last_login_at=NOW(3) WHERE id=?',[user.id]); res.status(204).end() } catch(e){ next(e) } })
