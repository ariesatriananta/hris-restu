...
employeesRouter.post('/contracts/:contractUid/print-snapshot', requirePermission('employees.manage'), async (req,res,next)=>{ const conn=await pool.getConnection();try{const auth=res.locals.auth as AuthContext;const uid=routeParam(req.params.contractUid);await conn.beginTransaction();const [rows]=await conn.query<RowDataPacket[]>(`SELECT c.id,c.uid,c.contract_number contractNumber,DATE_FORMAT(c.start_date,'%Y-%m-%d') startDate,DATE_FORMAT(c.end_date,'%Y-%m-%d') endDate,DATE_FORMAT(c.signed_date,'%Y-%m-%d') signedDate,c.terms_json termsJson,ct.code contractType,e.id employeeId,e.full_name fullName,e.employee_number employeeNumber,e.national_id_number nationalIdNumber,e.address,e.rtrw,e.kelurahan,e.kecamatan,e.city,e.province,s.code currentSite FROM employee_contracts c JOIN contract_types ct ON ct.id=c.contract_type_id JOIN employees e ON e.id=c.employee_id JOIN sites s ON s.id=e.current_site_id WHERE c.uid=? FOR UPDATE`,[uid]);const c=rows[0];if(!c)throw new ApiError(404,'Kontrak tidak ditemukan.');enforceSite(auth,c.currentSite);if(!['PKWT','TRAINING'].includes(c.contractType))throw new ApiError(422,'Cetak kontrak hanya tersedia untuk PKWT dan Training.');const terms=typeof c.termsJson==='string'?JSON.parse(c.termsJson||'{}'):c.termsJson??{};if(terms.contractPrintV1?.version==='PKWT_PRODUCTION_POSITION_V1'){await conn.commit();res.json(terms.contractPrintV1);return}if(!c.nationalIdNumber||!c.address)throw new ApiError(422,'NIK dan alamat karyawan wajib tersedia sebelum kontrak dicetak.');const [history]=await conn.query<RowDataPacket[]>(`SELECT s.id siteId,s.name siteName,s.address siteAddress,p.id positionId,p.name positionName FROM employee_employment_histories h JOIN sites s ON s.id=h.site_id LEFT JOIN positions p ON p.id=h.position_id WHERE h.employee_id=? AND h.effective_from<=? AND (h.effective_to IS NULL OR h.effective_to>=?) ORDER BY h.effective_from DESC,h.id DESC LIMIT 1`,[c.employeeId,c.startDate,c.startDate]);const site=history[0];if(!site?.siteAddress)throw new ApiError(422,'Alamat site wajib tersedia sebelum kontrak dicetak.');if(!site.positionId)throw new ApiError(422,'Posisi karyawan pada tanggal mulai kontrak wajib tersedia sebelum kontrak dicetak.');const [jobs]=await conn.query<RowDataPacket[]>(`SELECT j.uid jobUid,j.code jobCode,j.name jobName,u.name unitName,r.uid rateUid,r.rate_amount rateAmount,r.currency,DATE_FORMAT(r.effective_from,'%Y-%m-%d') effectiveFrom FROM production_jobs j JOIN production_job_rates r ON r.production_job_id=j.id AND r.site_id=? AND r.status='ACTIVE' AND r.effective_from<=? AND (r.effective_to IS NULL OR r.effective_to>=?) JOIN work_units u ON u.id=r.unit_id WHERE j.position_id=? AND j.is_active=1 ORDER BY j.name`,[site.siteId,c.startDate,c.startDate,site.positionId]);if(!jobs.length)throw new ApiError(422,'Belum ada pekerjaan dan tarif aktif untuk posisi karyawan pada tanggal mulai kontrak.');const address=[c.address,c.rtrw&&`RT/RW ${c.rtrw}`,c.kelurahan,c.kecamatan,c.city,c.province].filter(Boolean).join(', ');const snapshot={version:'PKWT_PRODUCTION_POSITION_V1',generatedAt:new Date().toISOString(),contract:{uid:c.uid,number:c.contractNumber,type:c.contractType,startDate:c.startDate,endDate:c.endDate,signedDate:c.signedDate},company:{name:'PT Restu Sejati Inti Abadi',site:site.siteName,address:site.siteAddress,signerTitle:'Kepala Produksi Site'},employee:{name:c.fullName,employeeNumber:c.employeeNumber,nationalIdNumber:c.nationalIdNumber,address,position:site.positionName??'Pekerja Produksi'},payment:{schedule:'Setiap dua minggu'},jobs:jobs.map(j=>({uid:j.jobUid,code:j.jobCode,name:j.jobName,unit:j.unitName,rateAmount:Number(j.rateAmount),currency:j.currency,rateUid:j.rateUid,effectiveFrom:j.effectiveFrom}))};await conn.execute('UPDATE employee_contracts SET terms_json=?,updated_by=? WHERE id=?',[JSON.stringify({...terms,contractPrintV1:snapshot}),auth.id,c.id]);await writeAudit({auth,request:req,siteId:site.siteId,action:'GENERATE',table:'employee_contracts',recordId:c.id,recordUid:uid,description:`Membuat snapshot cetak kontrak ${c.contractNumber}.`},conn);await conn.commit();res.json(snapshot)}catch(e){await conn.rollback();next(e)}finally{conn.release()} })
