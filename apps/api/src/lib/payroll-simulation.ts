    ...
    if (Number(run.deductBpjs) === 1) {
      ...
      await conn.execute(
        ...
        SELECT UUID(),result.id,settlement.id,policy.id,NULL,wage.id,enrollment.id,
          ...
          IF((CASE WHEN enrollment.configuration_mode='CUSTOM' THEN enrollment.health_employer_enabled ELSE policy.health_employer_enabled END)=1,ROUND(LEAST(wage.amount,COALESCE(policy.health_wage_ceiling,wage.amount))*policy.health_employer_rate/100,2),0),
          policy.health_employee_rate,
          IF((CASE WHEN enrollment.configuration_mode='CUSTOM' THEN enrollment.health_employee_enabled ELSE policy.health_employee_enabled END)=1,ROUND((LEAST(wage.amount,COALESCE(policy.health_wage_ceiling,wage.amount))*policy.health_employee_rate/100)/policy.rounding_unit,0)*policy.rounding_unit,0),
          policy.jht_employer_rate,
          IF((CASE WHEN enrollment.configuration_mode='CUSTOM' THEN enrollment.jht_employer_enabled ELSE policy.jht_employer_enabled END)=1,ROUND(wage.amount*policy.jht_employer_rate/100,2),0),
          policy.jht_employee_rate,
          IF((CASE WHEN enrollment.configuration_mode='CUSTOM' THEN enrollment.jht_employee_enabled ELSE policy.jht_employee_enabled END)=1,ROUND((wage.amount*policy.jht_employee_rate/100)/policy.rounding_unit,0)*policy.rounding_unit,0),
          policy.jkk_employer_rate,
          IF((CASE WHEN enrollment.configuration_mode='CUSTOM' THEN enrollment.jkk_employer_enabled ELSE policy.jkk_employer_enabled END)=1,ROUND(wage.amount*policy.jkk_employer_rate/100,2),0),
          policy.jkm_employer_rate,
          IF((CASE WHEN enrollment.configuration_mode='CUSTOM' THEN enrollment.jkm_employer_enabled ELSE policy.jkm_employer_enabled END)=1,ROUND(wage.amount*policy.jkm_employer_rate/100,2),0),
          policy.jp_employer_rate,
          IF((CASE WHEN enrollment.configuration_mode='CUSTOM' THEN enrollment.jp_employer_enabled ELSE policy.jp_employer_enabled END)=1,ROUND(LEAST(wage.amount,COALESCE(policy.jp_wage_ceiling,wage.amount))*policy.jp_employer_rate/100,2),0),
          policy.jp_employee_rate,
          IF((CASE WHEN enrollment.configuration_mode='CUSTOM' THEN enrollment.jp_employee_enabled ELSE policy.jp_employee_enabled END)=1,ROUND((LEAST(wage.amount,COALESCE(policy.jp_wage_ceiling,wage.amount))*policy.jp_employee_rate/100)/policy.rounding_unit,0)*policy.rounding_unit,0),
          0,0,
          JSON_OBJECT(
            'policyUid',policy.uid,
            'policyYear',policy.policy_year,
            'minimumWageUid',wage.uid,
            'minimumWage',wage.amount,
            'roundingUnit',policy.rounding_unit,
            'configurationMode',IF(enrollment.configuration_mode='CUSTOM','CUSTOM','GLOBAL'),
            'healthEmployerEnabled',IF((CASE WHEN enrollment.configuration_mode='CUSTOM' THEN enrollment.health_employer_enabled ELSE policy.health_employer_enabled END)=1,TRUE,FALSE),
            'healthEmployeeEnabled',IF((CASE WHEN enrollment.configuration_mode='CUSTOM' THEN enrollment.health_employee_enabled ELSE policy.health_employee_enabled END)=1,TRUE,FALSE),
            'jhtEmployerEnabled',IF((CASE WHEN enrollment.configuration_mode='CUSTOM' THEN enrollment.jht_employer_enabled ELSE policy.jht_employer_enabled END)=1,TRUE,FALSE),
            'jhtEmployeeEnabled',IF((CASE WHEN enrollment.configuration_mode='CUSTOM' THEN enrollment.jht_employee_enabled ELSE policy.jht_employee_enabled END)=1,TRUE,FALSE),
            'jkkEmployerEnabled',IF((CASE WHEN enrollment.configuration_mode='CUSTOM' THEN enrollment.jkk_employer_enabled ELSE policy.jkk_employer_enabled END)=1,TRUE,FALSE),
            'jkmEmployerEnabled',IF((CASE WHEN enrollment.configuration_mode='CUSTOM' THEN enrollment.jkm_employer_enabled ELSE policy.jkm_employer_enabled END)=1,TRUE,FALSE),
            'jpEmployerEnabled',IF((CASE WHEN enrollment.configuration_mode='CUSTOM' THEN enrollment.jp_employer_enabled ELSE policy.jp_employer_enabled END)=1,TRUE,FALSE),
            'jpEmployeeEnabled',IF((CASE WHEN enrollment.configuration_mode='CUSTOM' THEN enrollment.jp_employee_enabled ELSE policy.jp_employee_enabled END)=1,TRUE,FALSE)
          ),
          ?,?
        FROM payroll_employee_results result
