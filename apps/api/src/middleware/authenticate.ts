...
export async function authenticate(
  ...
  try {
    ...
    const [rows] = await pool.query<RowDataPacket[]>(
      `SELECT u.id,u.uid,u.full_name,u.email,u.status,u.must_change_password,r.code role_code,p.code permission_code,s.code site_code FROM users u JOIN user_sessions sess ON sess.user_id=u.id AND sess.uid=? AND sess.revoked_at IS NULL AND sess.expires_at>NOW(3) LEFT JOIN user_roles ur ON ur.user_id=u.id LEFT JOIN roles r ON r.id=ur.role_id AND r.is_active=1 LEFT JOIN role_permissions rp ON rp.role_id=r.id LEFT JOIN permissions p ON p.id=rp.permission_id LEFT JOIN user_site_access usa ON usa.user_id=u.id LEFT JOIN sites s ON s.id=usa.site_id WHERE u.uid=?`,
      [sessionUid, verified.payload.sub]
    )
    if (!rows[0] || rows[0].status !== 'ACTIVE')
      throw new ApiError(401, 'User tidak aktif.')
    res.locals.auth = {
      id: rows[0].id,
      uid: rows[0].uid,
      name: rows[0].full_name,
      email: rows[0].email,
      roles: [...new Set(rows.map((r) => r.role_code).filter(Boolean))],
      permissions: [
        ...new Set(rows.map((r) => r.permission_code).filter(Boolean)),
      ],
      siteAccess: [...new Set(rows.map((r) => r.site_code).filter(Boolean))],
      sessionUid,
      mustChangePassword: Boolean(rows[0].must_change_password),
    } satisfies AuthContext
    if (res.locals.auth.mustChangePassword) {
      const requestPath = req.originalUrl.split('?')[0]?.replace(/\/$/, '')
      const allowed =
        (req.method === 'GET' &&
          (requestPath === '/api/auth/me' ||
            requestPath === '/api/auth/profile')) ||
        (req.method === 'PATCH' && requestPath === '/api/auth/profile/password')
      if (!allowed)
    ...
    next(
      error instanceof ApiError
        ? error
        : new ApiError(401, 'Sesi tidak valid atau kedaluwarsa.')
    )
  }
}
